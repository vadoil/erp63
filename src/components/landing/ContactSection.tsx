    ...
    return (
      <section id="contact" className="section-padding">
        <div className="container max-w-2xl text-center space-y-6">
          ...
          <h2 className="text-3xl font-extrabold">Спасибо за заявку!</h2>
          <p className="text-muted-foreground text-lg">Мы свяжемся с вами в течение 2 рабочих часов для обсуждения вашего проекта.</p>
          <Button onClick={() => { setSubmitted(false); setForm({ name: "", phone: "", company: "", position: "", comment: "" }); setAgreed(false); }} variant="outline">
    ...
    <section id="contact" className="section-padding">
      <div className="container">
        <div className="text-center mb-14 max-w-2xl mx-auto">
          ...
          <p className="text-muted-foreground text-lg">Оставьте заявку — ответим в&nbsp;течение 2&nbsp;рабочих часов.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 bg-card border border-border rounded-xl p-8 shadow-sm">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input placeholder="Ваше имя" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} maxLength={100} />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Телефон *</label>
                <Input placeholder="+7 (___) ___-__-__" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} maxLength={20} />
              </div>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Компания</label>
                <Input placeholder="Название компании" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} maxLength={100} />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Должность</label>
                <Input placeholder="Ваша должность" value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} maxLength={100} />
              </div>
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">Комментарий</label>
              <Textarea placeholder="Расскажите о задаче..." value={form.comment} onChange={(e) => setForm({ ...form, comment: e.target.value })} rows={4} maxLength={1000} />
            </div>
              ...
              {[
                ...
                return (
                  <div key={i} className="flex items-start gap-4">
                    <div className="w-11 h-11 rounded-lg bg-yellow-soft flex items-center justify-center shrink-0">
